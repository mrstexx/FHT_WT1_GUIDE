...
function loadRoomData(roomNumber) {
    ...
    if (Number.isInteger(parseInt(roomNumber))) {
        ...
        var navStructure = '<thead><tr>'
            ...
            + '<th class="th-sm">Lectors</th>'
            + '<th class="th-sm">Groups</th>'
            + '<th class="th-sm">Subject</th>'
            + '<th class="th-sm">Note</th>'
            + '<th class="th-sm">Hour from</th>'
            + '<th class="th-sm">Hour to</th></tr></thead><tbody>';
        $.ajax({
            type: 'GET',
            dataType: 'xml',
...
function getList() {
    var listStructure = '<div class="form-group"><select class="form-control mx-auto" id="listSelection"><option value="0" selected>Choose a room</option>';
